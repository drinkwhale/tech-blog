...
글 유형에 따라 구조가 다르다. **먼저 유형을 정한 뒤 해당 구조를 따른다.**

| 글 유형             | 구조                                                                     | 적용 skill/템플릿                       |
| ------------------- | ------------------------------------------------------------------------ | --------------------------------------- |
| 트러블슈팅·사고기록 | STAR: TL;DR → 배경/문제 → 시도한 것들 → 해결 과정 → 결과(수치) → 배운 점 | `/write-post` + `archetypes/default.md` |
| 기술 개념 소개      | What → Why → How (+ Mermaid 2개·비교표)                                  | `tech-concept-post` skill               |
| 장문·voice 중심     | 자유 구조                                                                | `article-writing` skill                 |
...
4. **작성** — 글 유형에 맞는 skill 선택 (위 Writing Convention 표 참고)
   - 트러블슈팅·사고기록 → `/write-post` (STAR 초안)
   - 기술 개념 소개 → `tech-concept-post` skill (What/Why/How + Mermaid)
   - 장문·voice 중심 → `/article-writing`
